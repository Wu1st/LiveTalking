    ...
    def process_frames(self,quit_event):
        ...
        while not quit_event.is_set():
            ...
            for audio_frame in audio_frames:
                ...
                # 重采样滤波可能让原本位于 [-1, 1] 的浮点 PCM 产生少量
                # 越界峰值。直接转换为 int16 会整数回绕并形成爆音 / 电音，
                # 因此在送入 WebRTC 前再次清理、裁剪并安全量化。
                audio_data = np.asarray(audio_frame.data, dtype=np.float32)
                ...
                frame = np.rint(audio_data * 32767.0).astype(np.int16)
