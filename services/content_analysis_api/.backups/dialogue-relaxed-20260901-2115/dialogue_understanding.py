...
async def generate_dialogue_understanding(
    ...
    *,
    ...
    for attempt in range(3):
        ...
        errors = (
            _prose_safety_errors(candidate, facts)
            + _writer_errors(candidate, facts)
            + _style_errors(candidate)
        )
    ...
    if dialogue is None:
        dialogue = {
            ...
            # This is an internal quality-routing decision, not uncertainty in
            # the user's recording.  fallback_used remains available in the
            # API metadata for operations and regression monitoring.
            "uncertainties": [],
        }
