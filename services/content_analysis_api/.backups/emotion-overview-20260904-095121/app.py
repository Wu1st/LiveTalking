...
AnalysisMode = Literal["quick_summary", "multimodal_analysis"]
...
FULL_ANALYSIS_TASKS: tuple[AnalysisTask, ...] = (
    "digest",
    "summary",
    "qa",
    "relationships",
...
class AnalysisPlan:
    ...
    @property
    def attempted_tasks(self) -> list[str]:
        values = ["asr"]
        if self.run_diarization:
            values.append("diarization")
        if self.run_acoustic:
            values.append("acoustic")
        if self.run_emotion:
    ...
    @property
    def skipped_tasks(self) -> list[dict[str, str]]:
        result = []
        for name, enabled in (
            ("diarization", self.run_diarization),
            ("acoustic", self.run_acoustic),
            ("emotion", self.run_emotion),
        ):
            if not enabled:
                result.append({"task": name, "reason": "not_requested"})
        return result
ContentType = Literal[
    "auto",
    "meeting",
    ...
    def from_env(cls) -> "Settings":
        return cls(
            ...
            dialogue_primary_model=os.getenv(
                "CONTENT_ANALYSIS_DIALOGUE_PRIMARY_MODEL", "qwen2.5:7b"
            ).strip(),
            dialogue_primary_timeout_seconds=float(
...
class AcousticEvent(BaseModel):
    ...
    label_zh: str | None = Field(default=None, max_length=256)
    ...
    hit_window_count: int | None = Field(default=None, ge=0)


class AcousticContext(BaseModel):
    events: list[AcousticEvent] = Field(default_factory=list, max_length=2000)
    ...
    window_seconds: float | None = Field(default=None, ge=0)
    window_hop_seconds: float | None = Field(default=None, ge=0)
...
9. 若tasks包含digest，先根据内容判断meeting、interview、customer_service、lecture、
   ...
   topic_progression最多4项，每项narrative不超过80字、inference不超过40字；key_points最多5项；
   每个Speaker最多一条contribution，避免逐句复述和冗长输出。
10. 为降低实时分析延迟，只返回紧凑JSON对象，不输出Markdown，不复述完整scores，也不要逐句摘抄。
顶层只返回relationships、scene_analysis、content_digest、uncertainties、evidence_segment_ids；
不要返回answer、summary、speaker_analysis或emotion_analysis，这些字段由服务器根据事实统一补全。
...
COMPACT_ANALYSIS_FORMAT: dict[str, Any] = {
    "type": "object",
            ...
            "properties": {
                ...
                "action_items": {
                    ...
                    "items": {
                        ...
                        "required": ["owner", "task", "due", "evidence_segment_ids"],
                    },
                },
                "open_questions": {
                    "type": "array",
...
def _clean_display_text(value: Any, maximum: int) -> str:
    """Remove ASR boundary punctuation that should not lead UI text."""
...
def build_structured_input(
    ...
    *,
    ...
    if not isinstance(diarization, dict):
        diarization = {}
    diarization_available = bool(diarization.get("segments"))
    ...
    return StructuredAnalysisRequest(
        ...
        conversation=Conversation(
            speaker_count=(
                ...
                or len({segment.speaker for segment in segments})
            )
        ...
        acoustic_context=AcousticContext(
            events=acoustic_events,
            audio_duration_seconds=acoustic_payload.get("audio_duration_seconds")
            if isinstance(acoustic_payload, dict)
            else None,
            analyzed_duration_seconds=acoustic_payload.get("analyzed_duration_seconds")
            if isinstance(acoustic_payload, dict)
            else None,
            window_count=acoustic_payload.get("window_count")
            if isinstance(acoustic_payload, dict)
            else None,
            window_seconds=acoustic_payload.get("window_seconds")
            if isinstance(acoustic_payload, dict)
            else None,
            window_hop_seconds=acoustic_payload.get("window_hop_seconds")
            if isinstance(acoustic_payload, dict)
            else None,
        ),
        emotion=emotion,
...
def _guard_content_digest(
    ...
    *,
    ...
    overview = _clean_display_text(
        source.get("overview") or summary or "暂无内容概览。",
        1200,
    )
    key_points = [
        _clean_display_text(item, 500)
        for item in source.get("key_points") or []
        if _clean_display_text(item, 500)
    ...
    if not key_points:
        # A missing LLM field must not degrade into one bullet per short ASR
        # fragment.  The overview is a coherent, already-guarded fallback.
        key_points = [overview] if overview else []
...
def apply_grounding_guardrails(
    ...
    if value.conversation.diarization_available and len(speakers) >= 2:
    ...
    if value.conversation.diarization_available:
        ...
        result["speaker_analysis"] = {
            ...
            "speakers": [],
            "identity_attributes_inferred": False,
            "uncertainty": "说话人日志化未执行或执行失败，不能据此判断人数和人物关系。",
        }
    ...
    observed_events = [
        {
            ...
            "hit_window_count": item.hit_window_count,
        }
        for item in events
    ]
    ...
    if speech_only:
        result["scene_analysis"] = {
            ...
            "audio_duration_seconds": value.acoustic_context.audio_duration_seconds,
            "analyzed_duration_seconds": value.acoustic_context.analyzed_duration_seconds,
            "window_count": value.acoustic_context.window_count,
            "evidence_segment_ids": [],
        ...
        if not events:
            ...
            result["scene_analysis"] = {
                **scene,
                ...
                "analyzed_duration_seconds": value.acoustic_context.analyzed_duration_seconds,
                "window_count": value.acoustic_context.window_count,
                "window_seconds": value.acoustic_context.window_seconds,
                "window_hop_seconds": value.acoustic_context.window_hop_seconds,
                "evidence_segment_ids": _filter_evidence_ids(
...
async def _ollama_analyze(
    ...
    overview = digest.get("overview") if isinstance(digest, dict) else None
    analysis["dialogue_understanding"] = str(
        overview or analysis.get("summary") or analysis.get("answer") or ""
    ).strip()
...
def _build_analysis_plan(
    raw_mode: str | None,
    selected_tasks: list[AnalysisTask],
) -> AnalysisPlan:
    """Resolve one explicit execution plan before any upstream call starts."""
    requested = (raw_mode or "").strip().lower()
    if not requested:
        requested = (
            "quick_summary"
            if set(selected_tasks) <= {"digest", "summary"}
            else "multimodal_analysis"
        )
    if requested not in {"quick_summary", "multimodal_analysis"}:
        raise HTTPException(status_code=422, detail="unsupported analysis_mode")

    if requested == "quick_summary":
        return AnalysisPlan(
            mode="quick_summary",
    ...
    # Complete mode is intentionally complete.  This prevents a caller from
    # selecting the label while silently omitting speaker, emotion or scene.
    return AnalysisPlan(
        mode="multimodal_analysis",
...
def _upstream_result(
    ...
    *,
    livetalking_envelope: bool,
) -> tuple[dict[str, Any] | None, dict[str, str] | None]:
    """Normalize one independent upstream result without hiding failures."""
    if isinstance(result, BaseException):
        return None, {"task": task, "reason": str(result)[:500]}
    try:
        result.raise_for_status()
        payload = result.json()
        if livetalking_envelope:
            payload = _extract_live_data(payload)
        if not isinstance(payload, dict):
            raise ValueError("upstream returned a non-object response")
        return payload, None
    except (httpx.HTTPError, ValueError, TypeError) as exc:
        return None, {"task": task, "reason": str(exc)[:500]}
...
def _fallback_analysis(
    ...
    """Keep deterministic upstream evidence when the analysis LLM fails."""
    notice = "内容理解模型暂不可用，已保留成功的转写、说话人、情绪和场景结果。"
    evidence_ids = [segment.id for segment in value.conversation.segments]
    base = {
        "answer": notice,
        "summary": notice,
        "dialogue_understanding": notice,
        "content_digest": {
            "content_type": value.content_type_hint
            if value.content_type_hint != "auto"
            else "other",
            "overview": notice,
            "key_points": [],
    ...
    result = apply_grounding_guardrails(base, value)
    result["dialogue_understanding"] = notice
    result["dialogue_understanding_meta"] = {
...
def _analysis_without_transcript(
    acoustic_payload: dict[str, Any] | None,
    emotion_payload: dict[str, Any] | None,
    failures: list[dict[str, str]],
) -> dict[str, Any]:
    """Return non-text observations when both transcript sources fail."""
    raw_events = []
    if isinstance(acoustic_payload, dict):
        raw_events = (
            acoustic_payload.get("event_segments")
    ...
    emotion_analysis: dict[str, Any] = {"status": "unavailable"}
    if isinstance(emotion_payload, dict) and emotion_payload.get("emotion"):
        emotion_analysis = {
            "scope": "whole_audio",
            "observed_label": str(emotion_payload["emotion"]),
            "confidence": emotion_payload.get("confidence"),
            "speaker_attribution": None,
            "interpretation": "上游整段音频情绪标签；不能据此归因到单个说话人。",
        }
    notice = "语音转写和说话人文本均不可用，无法生成对话理解；其他成功结果已保留。"
    return {
        "answer": notice,
        "summary": notice,
        "dialogue_understanding": notice,
        ...
        "scene_analysis": {
            "status": "available" if observed_events else "unavailable",
            "observed_events": observed_events,
            "audio_duration_seconds": acoustic_payload.get("audio_duration_seconds")
            if isinstance(acoustic_payload, dict)
            else None,
        ...
        "uncertainties": [item["task"] + ": " + item["reason"] for item in failures],
        "evidence_segment_ids": [],
    }
...
async def model_info(request: Request) -> dict[str, Any]:
    ...
    return {
        ...
        "analysis_modes": ["quick_summary", "multimodal_analysis"],
    }
...
async def analyze_audio(
    ...
    analysis_mode: Annotated[str | None, Form(max_length=32)] = None,
    ...
    plan = _build_analysis_plan(analysis_mode, selected_tasks)
    selected_tasks = list(plan.tasks)
    ...
    async with request.app.state.slots:
        ...
        if plan.run_diarization:
            calls["diarization"] = client.post(
                f"{settings.livetalking_url}/diarize_audio",
                files={"file": (filename, audio, content_type)},
                data={"language": language},
                timeout=settings.upstream_timeout_seconds,
            )
        if plan.run_acoustic:
            calls["acoustic"] = client.post(
                f"{settings.livetalking_url}/analyze_environment",
                files={"file": (filename, audio, content_type)},
                data={
                    "analysis_session_id": request_id,
        ...
        LOGGER.info(
            "analysis plan request=%s mode=%s tasks=%s upstreams=%s",
            request_id,
            plan.mode,
            list(plan.tasks),
            list(calls),
        ...
        raw_results = await asyncio.gather(
            *calls.values(), return_exceptions=True
        )
        upstreams: dict[str, dict[str, Any]] = {}
        failed_tasks: list[dict[str, str]] = []
        successful_tasks: list[str] = []
        for name, raw_result in zip(calls, raw_results):
            payload, failure = _upstream_result(
                name,
                raw_result,
                livetalking_envelope=name != "emotion",
            )
            if failure is not None:
                failed_tasks.append(failure)
                LOGGER.warning(
                    "analysis upstream failed request=%s task=%s reason=%s",
                    request_id,
                    name,
                    failure["reason"],
                )
            elif payload is not None:
                upstreams[name] = payload
                successful_tasks.append(name)

        live_data: dict[str, Any] = {}
        asr_payload = upstreams.get("asr")
        if asr_payload:
            live_data.update(asr_payload)
        diarization_payload = upstreams.get("diarization")
        if diarization_payload:
            live_data["speaker_diarization"] = diarization_payload
            if not live_data.get("duration"):
                live_data["duration"] = diarization_payload.get("duration")
        acoustic_payload = upstreams.get("acoustic")
        if acoustic_payload:
            live_data["acoustic"] = acoustic_payload
            if not live_data.get("duration"):
                live_data["duration"] = acoustic_payload.get(
                    "audio_duration_seconds"
                )
        for failure in failed_tasks:
            if failure["task"] == "diarization":
                live_data["speaker_diarization_error"] = failure["reason"]
            elif failure["task"] == "acoustic":
                live_data["acoustic_error"] = failure["reason"]

        emotion_payload = upstreams.get("emotion")
        emotion_failure = next(
            (item for item in failed_tasks if item["task"] == "emotion"),
            None,
        )
        structured: StructuredAnalysisRequest | None = None
        model: dict[str, Any] | None = None
        try:
            structured = build_structured_input(
                {"code": 0, "data": live_data},
                emotion_payload,
                filename=filename,
                language=language,
                question=question,
                tasks=selected_tasks,
                emotion_error=emotion_failure["reason"]
                if emotion_failure
                else None,
            ...
            failed_tasks.append({"task": "transcript", "reason": str(exc)[:500]})

        if structured is None:
            analysis = _analysis_without_transcript(
                acoustic_payload,
                emotion_payload,
                failed_tasks,
            )
            ...
            try:
                ...
                successful_tasks.append("llm")
            except Exception as exc:
                failed_tasks.append({"task": "llm", "reason": str(exc)[:500]})
                analysis = _fallback_analysis(
                    structured,
                    f"llm: {str(exc)[:500]}",
                )
                LOGGER.exception("analysis LLM failed request=%s", request_id)
    ...
    LOGGER.info(
        "analysis complete request=%s mode=%s status=%s successful=%s failed=%s skipped=%s elapsed=%.3f",
        request_id,
        plan.mode,
        status,
        successful_tasks,
        [item["task"] for item in failed_tasks],
        [item["task"] for item in skipped_tasks],
        time.perf_counter() - started,
    )

    attempted_tasks = list(calls)
    if structured is not None:
        attempted_tasks.append("llm")
